function [eventStats, X] = find_LFP_events(cfg_in, lowpass, highpass)
...
%   eventDuration:       how long you want the gamma bouts to be (in s). This parameter is chosen to make them all the same length, but in reality, gamma bouts
...
%   X:                  nChannels x Samples x nGammaEvents           
                            ...
                            % nSamples = number of data points in a gamma event (proportional to eventDuration)
...
for iSess = startSess:endSess;
    ...
    cfg_filter = [];
    cfg_filter.f = [lowpass highpass];  % frequency band to filter in. So far, just using low gamma events [45 65]
    ...
    TimeStepsToChopOFF = 2*round(cfg_out.eventDuration*fs); % How many samples to ignore at the end of the session...so that the last gamma event doesn't exceed the last timestep
    numBoutSamples = round(cfg_out.eventDuration*fs/2);  % number of samples on either side of the peak of a gamma event to include 
    ...
    %% Build IV 
    eventStats = iv(gammaTimesStart, gammaTimesEnd); 
